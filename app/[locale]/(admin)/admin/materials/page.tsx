// app/[locale]/(admin)/admin/materials/page.tsx
"use client";
...
import { UpsertMaterialModal } from "@/components/modal/UpsertMaterialModal";
...
export default function AdminMaterialsPage() {
  ...
  useEffect(() => {
    if (isError && error) {
      toast.error(`Materyaller yüklenemedi: ${error.message}`);
    }
  }, [isError, error]);
  ...
  const filteredData = useMemo(() => 
    materials.filter(
      (material) =>
        ...
        material.lesson?.name.toLowerCase().includes(search.toLowerCase())
    ), [materials, search]
  );
  ...
  const columns: ColumnDef<Material>[] = useMemo(() => [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        ...
        >
          <Video className="w-3 h-3 mr-1.5 opacity-50" />
          {row.original.lesson?.name || "Bağımsız Materyal"}
        </Badge>
      ),
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex justify-end gap-2">
            <UpsertMaterialModal material={material}>
              <Button
                variant="ghost"
                size="icon"
              ...
              <AlertDialogContent className="rounded-[2rem] border-border/40">
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription className="text-base font-medium">
                    ...
                    materyali silinecektir. Bu işlem geri alınamaz.
                  </AlertDialogDescription>
                </AlertDialogHeader>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-muted-foreground mt-2 font-medium">
            Derslere ait PDF, çalışma kağıdı ve ek kaynakları buradan yönetin.
          </p>
        </div>
        <UpsertMaterialModal>
          <Button className="rounded-2xl h-14 px-8 font-black gap-2 shadow-xl shadow-primary/20 text-base">
            <Plus className="w-5 h-5" /> Yeni Materyal Ekle
          </Button>
        </UpsertMaterialModal>
      </div>
      ...
      <Card className="border-border/40 rounded-[2.5rem] bg-card overflow-hidden shadow-sm">
        <CardHeader className="px-8 pt-8 pb-4">
          <div className="relative group max-w-md">
            ...
            <Input
              placeholder="Materyallerde veya derslerde ara..."
